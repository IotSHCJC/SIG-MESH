...
一个总的消息类型，包括操作码，取决于下层传输层，它或许使用了分段和重组(SAR)机制。为了最大化性能并避免SAR开销，设计目标是将消息放入单个段中。传输层最多可为非分段消息提供11个字节，使用1个字节的操作码时，最多可为参数提供10个字节；使用2个字节的操作码时，最多可为9个字节提供参数。使用供应商特定的3字节操作码时，最多8个字节可用于参数。
传输层提供了一种SAR机制，能够传输多达32个段。使用SAR时，最大邮件大小为384个八位位组。这意味着（不包括应用程序MIC）使用1个八位字节的操作码时，参数最多可使用379个字节；使用2个字节的操作码时，参数最多可使用378个字节；当使用2个字节的操作码时，最多可使用377个字节。使用供应商特定的3字节操作码。
...
### 2.3.4 Elements
元素是节点内的可寻址实体。每个节点至少有一个元素，主要元素，或者还有一个或者多个添加的辅助元素。元素的数量和结构是固定的，在节点的整个生命周期内都不会改变。（即，只要该节点是网络的一部分）。
在供应期间，使用分配给节点的第一个单播地址来寻址主要元素。每个其他辅助元素都使用后续地址进行寻址。这些单播元素地址允许节点标识哪个节点的元素在传输或者接收一个数据。
举个例子由于固件升级，如果元素数量和结构发生改变，这个节点必须要被重新配置。执行固件更新以更改元素的数量或结构时，将使用“节点删除”过程（请参阅第3.10.7节）。
消息是根据操作码和元素地址在模型内分派的。
### 2.3.5 Addresses
